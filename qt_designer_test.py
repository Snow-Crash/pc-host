...
for i in range(DATA_SELECT_SYNAPSES):
    line = window.raw_data_plot.plot(np.random.rand(WINDOW), pen=colors[i%len(colors)])
...
for i in range(DATA_SELECT_NEURONS):
    line = window.voltage_plot.plot(np.random.rand(WINDOW), pen=colors[i%len(colors)])
